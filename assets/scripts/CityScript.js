...
cc.Class({
    ...
    _collisionTest: function(points, bullet){
        ...
        var ccv2 = cc.v2(parseInt(point.x / this._curMapTileSize.width), parseInt(point.y / this._curMapTileSize.height));
        var gid = this.mapLayer0.getTileGIDAt(ccv2);
        var tileType = this._tiledMapData.gidToTileType[gid];
        if (tileType != this._tiledMapData.tileType.tileNone && 
            tileType != this._tiledMapData.tileType.tileGrass){
            if(bullet){
                switch (tileType){
                    case this._tiledMapData.tileType.tileWall:
                        this.mapLayer0.setTileGIDAt(1, ccv2.x, ccv2.y, 1)
                        cc.log("hit a wall");
                        break;
                    case this._tiledMapData.tileType.tileKing:
                        cc.log("hit a king");
                        this.gameOver();
                        break;
                }
    ...
    addPlayerTank: function(team) {
        if(this.tankPool.size()>0){
            ...
            tankCtrl.die = false;
            tankCtrl.isPlayer = true;
            tankCtrl.mainCamera = this.mainCamera;
